...
function GymHome() {
    ...
    useEffect(()=>{
      if(!state.token){
        ...
        axios.get(`https://nutrifit-g0gn.onrender.com/gyms/owner/${state.userId}`,config).then((result) => {
          console.log(result);
        ...
        axios.get(`https://nutrifit-g0gn.onrender.com/gyms/user/${state.userId}`, config).then((result) => {
